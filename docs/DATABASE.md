...
- `card_comments` holds per-card discussion, `checklist_items` holds a card's steps, and `activities` is the board's audit trail.
- `columns` stores renameable columns and their board order; columns can be added, reordered, and deleted, and a board always keeps at least one.
...
Schema version 1 was approved and implemented in Part 6. Version 2 adds multiple boards per user and persistent sessions. Version 3 adds card due dates and assignees (added in place with `ALTER TABLE`) plus the `labels` and `card_labels` tables. Version 4 adds `board_members` (backfilled with an owner row for every existing board), `card_comments`, and `activities`. Version 5 adds `boards.archived_at`, so a board can be archived without losing its data. Version 6 adds `checklist_items`; the card payload carries `checklistDone`/`checklistTotal` so the board can show progress without a second request.
